...
std::optional<uint64_t> ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::
    reserve_linear_attention_checkpoints_for_next_step(uint64_t request_id, size_t num_checkpoints) {
    ...
    OPENVINO_ASSERT(running_sequences.size() == 1,
                    "Eagle3 linear-attention checkpointing requires one running sequence for request ",
                    request_id,
                    ", but found ",
...
void ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::
    promote_linear_attention_checkpoint_for_sequence(uint64_t sequence_id, size_t checkpoint_slot) {
    m_scheduler->promote_linear_attention_checkpoint(sequence_id, checkpoint_slot);
}

void ContinuousBatchingPipeline::ContinuousBatchingForSpeculativeDecodingImpl::
    release_linear_attention_checkpoints_for_sequence(uint64_t sequence_id) {
    m_scheduler->release_linear_attention_checkpoints(sequence_id);
}

void ContinuousBatchingPipeline::ContinuousBatchingForEagle3DecodingImpl::
    _commit_linear_attention_checkpoint_transactions(const Scheduler::Output& scheduler_output) {
